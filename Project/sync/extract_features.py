...
class vggFeatureModel(nn.Module):
            def __init__(self, original_model, depth):
                super(vggFeatureModel, self).__init__()
                self.features = nn.Sequential(
                    # stop at the second last layer
                    *list(original_model.features.children()),
                    Flatten(),
                    *list(original_model.classifier.children())[:depth]
                )
            def forward(self, x):
                x = self.features(x)
                return x
...
class resFeatureModel(nn.Module):
            def __init__(self, original_model, depth):
                ...
                self.features = nn.Sequential(
                    # stop at the second last layer
                    *list(original_model.children())[:depth],
                    Flatten()
                )
            def forward(self, x):
                x = self.features(x)
...
def run_model(modelname):
	...
	if modelname == 'vgg16':
		...
		model = vggFeatureModel(original_model, -1)
		total = 4096

		# vgg16.cuda()
	elif modelname == 'resnet':
		original_model = models.resnet152(pretrained=True)
		model = resFeatureModel(original_model, -1)
		total = 2048
	model.cuda()
	model.eval()

	#run_and_save('Train', total, model)
	run_and_save('Test', total, model)
	run_and_save('Valid', total, model)

def run_and_save(dataset, total, model):
	dirname = ''
	startInt = 0
	if dataset == 'Train':
		dirname = '../images/Train_crop/*/'
		startInt = 37
		savename = '../images/Train_res_feature/'
	elif dataset == 'Test':
		dirname = '../images/Test_crop/*/'
		startInt = 35
		savename = '../images/Test_res_feature/'
	elif dataset == 'Valid':
		dirname = '../images/Valid_crop/*/'
		startInt = 37
		savename = '../images/Valid_res_feature/'
